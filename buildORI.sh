...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	then
	    msg "|| Cloning modules ||"
	    git clone --depth 1 https://github.com/RandomiDn/iDn_modules Mod
	fi
}
...
exports() {
	...
	then
		KBUILD_COMPILER_STRING=$("$GCC64_DIR"/bin/aarch64-elf-ld --version | head -n 1)
...
build_kernel() {
	...
	if [ "$PTTG" = 1 ]
 	then
		tg_post_msg "<b>🔌Builder: </b><code>☬Ivan_Ssl$AUTHOR</code>%0A<b>🔌Straight: </b><code>[#$KBUILD_BUILD_VERSION] With [$COMPILER]</code>%0A<b>🔌Machine: </b><code>$DISTRO</code>%0A<b>🔌Kernel: </b><code>$KERVER</code>%0A<b>🔌Support: </b><code>$SUPPORT</code>%0A<b>🔌HostCore: </b><code>$PROCS</code>%0A<b>🔌Platform: </b><code>$VENDOR</code>%0A<b>🔌ConJobs: </b><code>$CONJOBS $(uname | awk -F: '{ print $1 }')</code>%0A<b>🔌Tools: </b><code>$KBUILD_COMPILER_STRING</code>%0A<b>🔌Branch: </b><code>$CI_BRANCH</code>%0A<b>🔌Commit: </b><code>$COMMIT_HEAD</code>%0A<b>🔌On: </b>[<a href='https://t.me/Random_iDn'>Group</a>] | [<a href='$SERVER_URL'><a href='https://t.me/RandomiDn'>©Channel</a>]</a>"
	fi
	...
	then
		...
		then
			...
			tg_post_msg "<code>|✅|Build Done</code>"
			if [ $BUILD_DTBO = 1 ]
			then
				msg "|| Building DTBO ||"
...
gen_zip() {
	...
	cdir $Anykernel3
	zip -r $ZIPNAME-$DEVICE-Ver#"$KBUILD_BUILD_VERSION"-$SUPPORT . -x ".git*" -x "README.md" -x "*.zip"
	if [ $MODULES = "1" ]
	then
		tg_post_msg "<code>|🗝|Add ModKernel </code>"
	    cdir $Mod
	    rm -rf system/lib/modules/placeholder
	    zip -r Mod$ZIPNAME-$DEVICE-Ver#"$KBUILD_BUILD_VERSION"-$SUPPORT . -x ".git*" -x "LICENSE.md" -x "*.zip"
	    	MOD_NAME="Mod$ZIPNAME-$DEVICE-Ver#$KBUILD_BUILD_VERSION-$SUPPORT"
	    cdir $AnyKernel3
	fi

	## Prepare a final zip variable
	ZIP_FINAL="$ZIPNAME-$DEVICE-Ver#$KBUILD_BUILD_VERSION-$SUPPORT"
...
#
	...
	if [ "$PTTG" = 1 ]
 	then
	    tg_post_build "$ZIP_FINAL.zip" "🛠Build Success. Kernel for Device: #riva / #rolex / #msm8937 (build took: $((DIFF / 60))/$((DIFF % 60)))"
	    if [ $MODULES = "1" ]
	    then
		cd $Mod
		tg_post_build "$MOD_NAME.zip" "🛠Modkernel Success. (Flash zip on magisk ModKernel)"
	    fi
	fi
	cd ..
}
