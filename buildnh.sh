...
#
	...
	if [ "$PTTG" = 1 ]
 	then
	    tg_post_build "$ZIP_FINAL.zip" "🛠Build Success. Kernelist for Device: $MODEL (build took: $((DIFF / 60))/$((DIFF % 60)))"
	    if [ $MODULES = "1" ]
	    then
		cd $Mod
