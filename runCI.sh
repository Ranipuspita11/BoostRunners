...
gen_zip() {
	...
	cdir $Anykernel3
	zip -r $ZIPNAME-$DEVICE-"$KBUILD_BUILD_VERSION"-Carnage . -x ".git*" -x "README.md" -x "*.zip"
	if [ $MODULES = "1" ]
	then
	    cdir ../Mod
	    rm -rf system/lib/modules/placeholder
	    zip -r $ZIPNAME-$DEVICE-modules-Carnage . -x ".git*" -x "LICENSE.md" -x "*.zip"
	    MOD_NAME="$ZIPNAME-$DEVICE-modules-Carnage"
	    cdir ../AnyKernel3
	fi

	## Prepare a final zip variable
	ZIP_FINAL="$ZIPNAME-$DEVICE-$KBUILD_BUILD_VERSION-Carnage"
