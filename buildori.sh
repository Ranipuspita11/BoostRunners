...
gen_zip() {
	...
	cdir $Anykernel3
	zip -r $ZIPNAME-$DEVICE-#"$KBUILD_BUILD_VERSION"-$SUPPORT . -x ".git*" -x "README.md" -x "*.zip"
	if [ $MODULES = "1" ]
	then
		tg_post_msg "<code>|🗝|Add ModKernel </code>"
	    cdir $Mod
	    rm -rf system/lib/modules/placeholder
	    zip -r Mod$ZIPNAME-$DEVICE-#"$KBUILD_BUILD_VERSION"-$SUPPORT . -x ".git*" -x "LICENSE.md" -x "*.zip"
	    cdir ../$AnyKernel3
	fi

	## Prepare a final zip variable
	ZIP_FINAL="$ZIPNAME-$DEVICE-#$KBUILD_BUILD_VERSION-$SUPPORT"
	MOD_NAME="Mod$ZIPNAME-$DEVICE-#$KBUILD_BUILD_VERSION-$SUPPORT"
#
	if [ $SIGN = 1 ]
	then
